...
4. Return / Q-function
    ...
    —> 에이전트는 Q-함수를 사용하여, 가능한 모든 행동 중 가장 높은 기대 보상을 주는 행동($\text{argmax} Q(s,a)$)를 선택하여 최적의 결정을 내릴 수 있다.
...
- Value Learning : **Q-함수와 Deep Q-Networks (DQN)**
    ...
    에이전트의 최적 정책 $\pi^*(s)$은 모든 가능한 행동 중 Q-값을 최대화하는 행동을 선택하는 것입니다.
...
- Deep Q-Network
    ...
    - 상태공간/행동공간
        ...
        ![빨리 기다리기.jpeg](/assets/img/DRLR/빨리_기다리기.jpeg)
        
    
    **학습 원리 (Bellman Equation):** DQN은 벨만 방정식(Bellman Equation)에 기초하여 학습한다. 네트워크가 예측한 현재의 Q-값(Predicted)과 실제 보상 및 다음 상태의 최대 Q-값을 합친 목표값 $(Target, r+\gamma \max_{a'} Q(s',a'))$사이의 오차인 **Q-Loss**를 줄이는 것이 핵심
    
    $$Loss = || (r + \gamma \max_{a'} Q(s', a')) - Q(s,a) ||^2$$
    
    ![5.png](/assets/img/DRLR/5.png)
...
![7.png](/assets/img/DRLR/7.png)

PG(policy gradient)의 손실 함수

$$Loss = - \log P(a_t \mid s_t) R_t$$

**마이너스 부호**가 붙는 이유는 딥러닝 최적화의 특성 때문. 누적 보상($R_t$)을 최대화(Maximize)하고자 하지만, 표준적인 경사 하강법(Gradient Descent)은 손실을 최소화(Minimize)하는 방향으로 작동한다. 
...
![말풍선 안보이면 다크모드로…](/assets/img/DRLR/만약_에이전트가_나쁜_행동을_반복하면_어떻게_학습하나요.png)
...
![출처. Jaden williams LIVE YouTube](/assets/img/DRLR/Car_accident.jpg)
...
- 하이브리드 학습(AlphaGo)
    ...
    1. **초기 학습 (Supervised Learning):** 인간 전문가의 기보를 바탕으로 특정 수에 대한 **Classification(분류)** 학습을 수행하여 기초 정책을 형성했다.
    
    ![2.png](/assets/img/DRLR/2.png)
    
    2. **자기 대국 (Self-play RL):** 이후 강화학습을 통해 스스로와 대국하며 데이터를 생성하고, 가치 네트워크를 통해 국면의 승률을 예측하는 **Regression(회귀)** 학습을 병행하여 초인적 성능에 도달했다.
